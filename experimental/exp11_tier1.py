#!/usr/bin/env python3
"""
EXP11: Tier 1 Complete (Dispersion + IC Scale + Decay Weight)
==============================================================
Combines all Tier 1 improvements:
...
- Dispersion scaling: reduce exposure when cross-sectional std is high
- Rolling IC scaling: reduce exposure when signal quality is low
- Decay-weighted IC estimation: recent IC matters more than old IC
"""
...
OUT_DIR       = "./experimental/exp11_tier1"
...
ROLLING_IC_WINDOW = 60   # 60-day rolling IC
IC_FLOOR          = 0.03 # below this = statistically noise (2σ for N=26)
IC_SCALE_FLOOR    = 0.20 # never go below 20% exposure
IC_WARMUP         = 120  # min days before IC estimation trusted
...
def compute_dispersion_scale(returns: pd.DataFrame) -> pd.Series:
    """Compute dispersion scale based on cross-sectional std."""
    # Daily cross-sectional std
    daily_cs_std = returns.std(axis=1)
    
    # Smooth over 5 days
    cs_std_smooth = daily_cs_std.rolling(CS_STD_SMOOTHING, min_periods=1).mean()
    
    # Expanding median (no lookahead)
    cs_std_median = cs_std_smooth.expanding(min_periods=60).median()
...
def compute_decay_weights(n: int, halflife: int = 365) -> np.ndarray:
    """Compute decay weights for IC estimation."""
    days_ago = np.arange(n - 1, -1, -1)
    weights = np.exp(-np.log(2) * days_ago / halflife)
    return weights / weights.sum()


def compute_ic_scale(scores: pd.DataFrame, returns: pd.DataFrame, all_dates: pd.DatetimeIndex) -> pd.Series:
    """Compute rolling IC scale with decay-weighted estimation."""
    print("Computing IC scale...")
    ...
    for i, dt in enumerate(all_dates[:-1]):
        ...
        if dt not in scores.index or next_dt not in returns.index:
            continue
        s = scores.loc[dt].dropna()
        r = returns.loc[next_dt].dropna()
    ...
    if len(ic_series) < IC_WARMUP:
        print(f"  ⚠️ Not enough IC data ({len(ic_series)} < {IC_WARMUP}), using default scale")
        return pd.Series(1.0, index=all_dates)
    
    ic_s = pd.Series(ic_series, index=pd.DatetimeIndex(ic_dates))
    
    # Rolling 60d IC mean
    rolling_ic = ic_s.rolling(ROLLING_IC_WINDOW, min_periods=20).mean()
    
    # Decay-weighted expanding IC mean (recent IC matters more)
    ic_scale_dict = {}
    for i, dt in enumerate(ic_dates):
        if i < IC_WARMUP:
            ic_scale_dict[dt] = 1.0
            continue
        
        # Use all IC up to this point
        ic_vals = np.array(ic_series[:i+1])
        decay_w = compute_decay_weights(len(ic_vals))
        weighted_ic_mean = np.sum(ic_vals * decay_w)
        
        # Scale = rolling / weighted_mean
        rolling_val = rolling_ic.get(dt, np.nan)
        if pd.isna(rolling_val):
            ic_scale_dict[dt] = 1.0
        else:
            ic_ratio = rolling_val / max(abs(weighted_ic_mean), IC_FLOOR)
            ic_scale_dict[dt] = max(min(ic_ratio, 1.0), IC_SCALE_FLOOR)
    
    ic_scale = pd.Series(ic_scale_dict)
    print(f"IC scale: min={ic_scale.min():.2f}, mean={ic_scale.mean():.2f}, max={ic_scale.max():.2f}")
    return ic_scale
...
def apply_leverage(
    ...
    """
    Apply vol-targeting leverage + dual regime scaling + dispersion + IC scale.
    Returns (scaled_weights, fast_scale, slow_scale, regime_scale, final_lev)
    """
    if portfolio_vol <= 0:
        lev = 1.0
    else:
    ...
    # Apply all scales
    final_lev = lev * regime_scale * disp_scale * ic_scale
    return w * final_lev, fast_scale, slow_scale, regime_scale, final_lev
...
def run_simulation(
    ...
    btc_vol_slow_median: float,
    btc_mom: pd.Series,
    disp_scale_series: pd.Series,
    ic_scale_series: pd.Series,
) -> tuple:
    print("\n=== EXP11: TIER 1 COMPLETE (Disp + IC Scale + Decay Weight) ===")
    ...
    print(f"Dispersion: threshold={DISP_THRESHOLD:.0%}, floor={DISP_SCALE_FLOOR:.0%}")
    print(f"IC scale: window={ROLLING_IC_WINDOW}d, floor={IC_SCALE_FLOOR:.0%}\n")
    ...
    for i, date in enumerate(all_dates[:-1]):
        ...
        # Get dispersion and IC scales
        disp_scale_today = disp_scale_series.get(date, 1.0)
        if pd.isna(disp_scale_today):
            disp_scale_today = 1.0
        
        ic_scale_today = ic_scale_series.get(date, 1.0)
        if pd.isna(ic_scale_today):
            ic_scale_today = 1.0
        ...
        if pause_counter > 0:
            ...
            current_w = pd.Series(dtype=float)
            fast_scale, slow_scale, regime_scale, final_lev = 1.0, 1.0, 1.0, 1.0
        else:
        ...
        pnl_rows.append({
            ...
            "disp_scale":   disp_scale_today,
            "ic_scale":     ic_scale_today,
        })
        ...
        if i % 100 == 0:
            print(f"  [{date.date()}] NAV={nav:,.0f}  DD={dd:.1%}  "
                  ...
                  f"disp={disp_scale_today:.2f}  ic={ic_scale_today:.2f}")
...
def compute_metrics(pnl_df: pd.DataFrame) -> dict:
    ...
    # Dispersion and IC stats
    avg_disp_scale = pnl_df["disp_scale"].mean() if "disp_scale" in pnl_df else 1.0
    avg_ic_scale   = pnl_df["ic_scale"].mean() if "ic_scale" in pnl_df else 1.0
    n_days_disp_reduced = (pnl_df["disp_scale"] < 0.8).sum() if "disp_scale" in pnl_df else 0
    n_days_ic_reduced   = (pnl_df["ic_scale"] < 0.8).sum() if "ic_scale" in pnl_df else 0
    ...
    return {
        ...
        "avg_ic_scale":    avg_ic_scale,
        "n_days_disp_reduced": n_days_disp_reduced,
        "n_days_ic_reduced":   n_days_ic_reduced,
    }
...
def create_charts(pnl_df: pd.DataFrame, metrics: dict):
    ...
    ax1.set_title(
        f"EXP11: Tier 1 Complete (Disp + IC Scale + Decay Weight)\n"
    ...
    ax4.plot(pnl_df["disp_scale"], color="purple", linewidth=1, label="Dispersion scale")
    ax4.plot(pnl_df["ic_scale"], color="green", linewidth=1, label="IC scale")
    ax4.axhline(1.0, color="black", linewidth=0.5)
    ax4.axhline(DISP_SCALE_FLOOR, color="purple", linewidth=0.8, linestyle="--")
    ax4.axhline(IC_SCALE_FLOOR, color="green", linewidth=0.8, linestyle="--")
    ...
    ax5.plot(pnl_df["btc_mom"], color="orange", linewidth=1, label="BTC 20d momentum")
    ax5.axhline(BTC_MOM_THRESHOLD, color="green", linewidth=1.5, linestyle="--",
                label=f"Gate threshold ({BTC_MOM_THRESHOLD:.0%})")
    ax5.axhline(0, color="black", linewidth=0.5)
    gate_mask = pnl_df["gate_active"]
    if gate_mask.any():
        ax5.fill_between(pnl_df.index, -0.5, 1.0, where=gate_mask, 
                         color="green", alpha=0.2, label="Gate active")
    ax5.set_ylabel("BTC Momentum")
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP11: TIER 1 COMPLETE (Disp + IC Scale + Decay Weight) ===\n")
        ...
        f.write(f"Dispersion: threshold={DISP_THRESHOLD:.0%}, floor={DISP_SCALE_FLOOR:.0%}\n")
        f.write(f"IC scale: window={ROLLING_IC_WINDOW}d, floor={IC_SCALE_FLOOR:.0%}, warmup={IC_WARMUP}d\n\n")
        ...
        f.write(f"Days disp < 0.8:       {metrics['n_days_disp_reduced']} days\n")
        f.write(f"Days IC < 0.8:         {metrics['n_days_ic_reduced']} days\n")
        f.write(f"Avg dispersion scale:  {metrics['avg_disp_scale']:.2f}\n")
        f.write(f"Avg IC scale:          {metrics['avg_ic_scale']:.2f}\n\n")
...
def main():
    ...
    # Compute dispersion scale
    disp_scale_series = compute_dispersion_scale(returns)

    # Compute IC scale
    all_dates = scores.index.intersection(returns.index)
    ic_scale_series = compute_ic_scale(scores, returns, all_dates)
    ...
    positions, pnl_df = run_simulation(
        ...
        btc_vol_fast_median, btc_vol_slow_median,
        btc_mom, disp_scale_series, ic_scale_series
    )
    ...
    print(f"\n  Avg disp scale: {metrics['avg_disp_scale']:.2f}")
    print(f"  Avg IC scale:   {metrics['avg_ic_scale']:.2f}")
    print(f"  Days disp < 0.8: {metrics['n_days_disp_reduced']}")
    print(f"  Days IC < 0.8:   {metrics['n_days_ic_reduced']}")

    # Charts + save
    create_charts(pnl_df, metrics)
    save_outputs(pnl_df, positions, metrics, monthly, attribution)

    print("\nDone. Read experimental/exp11_tier1/portfolio_metrics.txt for full results.")
