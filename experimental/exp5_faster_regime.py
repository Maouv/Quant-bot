#!/usr/bin/env python3
"""
EXP5: Faster Regime Filter (Dual Window)
=========================================
Copy of portofolio.py with:
- BTC_VOL_FAST = 10 (fast detector — catch danger quickly)
- BTC_VOL_SLOW = 60 (slow scaler — gradual position scaling)
- Dual window logic:
  - Fast trigger: if vol_fast > 2x median → cut to 50% immediately
  - Slow scaler: gradual (same as before)
  - Final regime scale = fast × slow
"""
...
OUT_DIR       = "./experimental/exp5_faster_regime"
...
# Regime filter - MODIFIED (dual window)
BTC_VOL_FAST = 10   # fast detector — catch danger quickly
BTC_VOL_SLOW = 60   # slow scaler — gradual position scaling
...
def load_btc_vol(returns: pd.DataFrame) -> tuple:
    """
    Compute BTC dual-window realized vol (annualized) for regime filter.
    Returns: (btc_vol_fast, btc_vol_slow, btc_vol_fast_median, btc_vol_slow_median)
    """
...
def run_simulation(
    ...
    """Main simulation loop with dual-window regime filter."""
    print(f"\n=== EXP5: FASTER REGIME FILTER (DUAL WINDOW) ===")
...
def create_charts(pnl_df: pd.DataFrame, metrics: dict):
    ...
    ax1.set_title(
        f"EXP5: Faster Regime Filter (Dual Window)\n"
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP5: FASTER REGIME FILTER (DUAL WINDOW) ===\n")
