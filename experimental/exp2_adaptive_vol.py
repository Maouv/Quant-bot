#!/usr/bin/env python3
"""
EXP2: Adaptive Vol Targeting (Dual EWMA)
=========================================
Same as portfolio.py but with dual EWMA for vol estimation:
- Fast EWMA (halflife=10): quick risk-off during crashes
- Slow EWMA (halflife=60): gradual risk-on during recovery
- Final vol estimate = MAX(fast, slow) → conservative

Expected: Faster deleverage in crashes, slower re-leverage in recovery.
"""
...
OUT_DIR       = "./experimental/exp2_adaptive_vol"

# Vol targeting
VOL_TARGET    = 0.15          # 15% annualized
EWMA_HALFLIFE_FAST = 10       # quick risk-off
EWMA_HALFLIFE_SLOW = 60       # slow risk-on
...
def run_simulation(
    ...
    EXP2: Uses dual EWMA for portfolio vol estimation.
    """
    print("\n=== RUNNING SIMULATION (EXP2: DUAL EWMA VOL) ===")
    print(f"Fast HL: {EWMA_HALFLIFE_FAST}, Slow HL: {EWMA_HALFLIFE_SLOW}")
    ...
    # Portfolio vol estimate (Dual EWMA)
    port_vol_fast = VOL_TARGET / np.sqrt(365)  # start at target
    port_vol_slow = VOL_TARGET / np.sqrt(365)  # start at target
    ...
    for i, date in enumerate(all_dates[:-1]):  # last day has no next-day return
        ...
        if pause_counter > 0:
            ...
            if len(target_w_raw) == 0:
                ...
                # Use max(fast, slow) for conservative vol estimate
                port_vol_ewm = max(port_vol_fast, port_vol_slow)
        ...
        # Update portfolio vol (Dual EWMA)
        alpha_fast = 1 - np.exp(-np.log(2) / EWMA_HALFLIFE_FAST)
        alpha_slow = 1 - np.exp(-np.log(2) / EWMA_HALFLIFE_SLOW)
        port_vol_fast = np.sqrt(alpha_fast * net_ret**2 + (1 - alpha_fast) * port_vol_fast**2)
        port_vol_slow = np.sqrt(alpha_slow * net_ret**2 + (1 - alpha_slow) * port_vol_slow**2)
        port_vol_ewm = max(port_vol_fast, port_vol_slow)  # conservative: take max
        ...
        pnl_rows.append({
            ...
            "port_vol_fast": port_vol_fast * np.sqrt(365),
            "port_vol_slow": port_vol_slow * np.sqrt(365),
...
def create_charts(pnl_df: pd.DataFrame, metrics: dict):
    ...
    gs  = gridspec.GridSpec(4, 1, height_ratios=[3, 1.5, 1.5, 1.5], hspace=0.35)
    ...
    ax1.set_title(
        f"EXP2: Dual EWMA Vol Targeting\n"
    ...
    # Dual EWMA vol comparison
    ax4 = fig.add_subplot(gs[3])
    ax4.plot(pnl_df["port_vol_fast"], color="red", linewidth=1, alpha=0.7, label=f"Fast EWMA (HL={EWMA_HALFLIFE_FAST})")
    ax4.plot(pnl_df["port_vol_slow"], color="green", linewidth=1, alpha=0.7, label=f"Slow EWMA (HL={EWMA_HALFLIFE_SLOW})")
    ax4.plot(pnl_df["port_vol_ann"], color="black", linewidth=1.5, label="Final vol (max)")
    ax4.axhline(VOL_TARGET, color="blue", linewidth=0.8, linestyle="--", label=f"Target ({VOL_TARGET:.0%})")
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP2: DUAL EWMA VOL TARGETING ===\n")
        f.write(f"Fast HL: {EWMA_HALFLIFE_FAST}, Slow HL: {EWMA_HALFLIFE_SLOW}\n")
...
def main():
    ...
    print("\nDone. Read experimental/exp2_adaptive_vol/portfolio_metrics.txt for full results.")
