#!/usr/bin/env python3
"""
EXP4: Symmetric Position Caps (10%/10%)
========================================
Copy of portofolio.py with:
- MAX_POS_PER_SYMBOL = 0.10 (was 0.12)
- MAX_SHORT_PER_SYMBOL = 0.10 (was 0.06) — symmetric now

Rationale: Under-allocating short alpha due to asymmetric cap.
liquidity and taker_buy_contrarian have good IC in short direction
but were capped too small. Test if symmetric 10/10 performs better.
"""
...
OUT_DIR       = "./experimental/exp4_symmetric_cap"
...
def compute_target_weights(
    ...
    """Compute target weights with symmetric caps."""
    ...
    if len(shorts) > 0:
        ...
        w_short = w_short.clip(lower=-MAX_SHORT_PER_SYMBOL)  # symmetric with longs now
...
def run_simulation(
    ...
    print(f"\n=== EXP4: SYMMETRIC CAPS 10%/10% ===")
    print(f"Long cap: {MAX_POS_PER_SYMBOL}, Short cap: {MAX_SHORT_PER_SYMBOL}\n")
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP4: SYMMETRIC CAPS 10%/10% ===\n")
        f.write(f"Long cap: {MAX_POS_PER_SYMBOL}, Short cap: {MAX_SHORT_PER_SYMBOL}\n")
