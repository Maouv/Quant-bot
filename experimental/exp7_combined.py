#!/usr/bin/env python3
"""
EXP7: COMBINED (Buffer 0.05 + Dual Regime)
==========================================
Combines:
- From exp3: TURNOVER_BUFFER = 0.05 (was 0.02), entry_date tracking
- From exp5: BTC_VOL_FAST = 10, BTC_VOL_SLOW = 60, dual window regime filter

NOTE: Does NOT include MIN_HOLD_DAYS from exp3 (hurts profit factor)
"""
...
OUT_DIR       = "./experimental/exp7_combined"
...
# Regime filter - FROM EXP5 (dual window)
BTC_VOL_FAST = 10   # fast detector
BTC_VOL_SLOW = 60   # slow scaler
...
def load_btc_vol(returns: pd.DataFrame) -> tuple:
    """Compute BTC dual-window realized vol for regime filter."""
...
def apply_leverage(
    ...
    """
    Scale weights by vol-targeting leverage × dual-window regime scaler.
    ...
    # Fast trigger: if vol_fast > 2x median → cut to 50%
...
def apply_turnover_buffer(
    ...
    Track entry_date for diagnostics (no min hold enforcement).
    ...
    # Track entry dates (diagnostics only, no min hold enforcement)
...
def run_simulation(
    ...
    """Main simulation loop with buffer 0.05 + dual regime filter."""
    print(f"\n=== EXP7: COMBINED (Buffer 0.05 + Dual Regime) ===")
    print(f"Buffer: {TURNOVER_BUFFER}")
    print(f"Fast window: {BTC_VOL_FAST}d, Slow window: {BTC_VOL_SLOW}d")
    print(f"Fast trigger: 2.0x median → cut 50%\n")
    ...
    entry_date      = {}  # track entry date per symbol
    pause_counter   = 0
...
def create_charts(pnl_df: pd.DataFrame, metrics: dict):
    ...
    ax1.set_title(
        f"EXP7: Combined (Buffer 0.05 + Dual Regime)\n"
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP7: COMBINED (Buffer 0.05 + Dual Regime) ===\n")
        f.write(f"Buffer: {TURNOVER_BUFFER}\n")
        f.write(f"Fast window: {BTC_VOL_FAST}d, Slow window: {BTC_VOL_SLOW}d\n")
        f.write("Fast trigger: 2.0x median → cut 50%\n")
        ...
        f.write(f"Fast Triggers:    {metrics['n_fast_triggers']:>10}\n")
...
def main():
    ...
    print(f"  Fast Triggers:{metrics['n_fast_triggers']:>8}")
