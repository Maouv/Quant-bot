#!/usr/bin/env python3
"""
EXP1: Conviction Filter Experiment
===================================
Copy of portofolio.py with conviction filter added.
Only change: filter scores by |score| > CONVICTION_THRESHOLD before vol-scaling.
"""
...
OUT_DIR       = "./experimental/result"

# Conviction filter
CONVICTION_THRESHOLD = 0.5  # sweep: [0.3, 0.5, 0.8, 1.0, 1.5]
...
def compute_target_weights(
    ...
    """Compute target weights with conviction filter."""
    ...
    # === CONVICTION FILTER ===
    s = s[s.abs() > CONVICTION_THRESHOLD]
...
def run_simulation(
    ...
    for i, date in enumerate(all_dates[:-1]):
        ...
        if i % 100 == 0:
            print(f"  [{date.date()}] NAV={nav:,.0f}  DD={dd:.1%}  Gross={gross_exp:.1f}x  n={n_long}L/{n_short}S")
    ...
    avg_positions = total_positions / days_with_trades if days_with_trades > 0 else 0
    ...
    return position_df, pnl_df, avg_positions


# ── METRICS ───────────────────────────────────────────────────────────────────

def compute_metrics(pnl_df: pd.DataFrame, avg_positions: float) -> dict:
...
def create_charts(pnl_df: pd.DataFrame, metrics: dict):
    ...
    ax1.set_title(
        f"EXP1: Conviction Filter (threshold={CONVICTION_THRESHOLD})\n"
...
def save_outputs(pnl_df, positions, metrics, monthly, attribution):
    ...
    with open(f"{OUT_DIR}/portfolio_metrics.txt", "w") as f:
        f.write("=== EXP1: CONVICTION FILTER EXPERIMENT ===\n")
        f.write(f"(Conviction threshold = {CONVICTION_THRESHOLD})\n\n")
        ...
        f.write(f"Conviction Threshold: {metrics['conviction_threshold']:>8.2f}\n")
        f.write(f"Avg Positions/Day:    {metrics['avg_positions_per_day']:>8.1f}\n\n")
...
def main():
    ...
    positions, pnl_df, avg_positions = run_simulation(scores, returns, btc_vol_series, btc_vol_median)
    ...
    print("\n=== COMPUTING METRICS ===")
    metrics     = compute_metrics(pnl_df, avg_positions)
    ...
    print(f"  Avg Pos/Day:  {metrics['avg_positions_per_day']:>8.1f}")
